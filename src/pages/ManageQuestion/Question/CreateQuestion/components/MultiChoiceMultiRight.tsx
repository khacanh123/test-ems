...
import { getImageLink } from 'api/requestAPI';
...
import { answerMultiRightType } from '../../type';

type MultiChoiceMultiRightProps = {
...
const MultiChoiceMultiRight = ({
  ...
  const [answerList, setAnswerList] = useState<answerMultiRightType[]>(firstRender());
  ...
  const handleChangeAnswer = (id: string, data: answerMultiRightType) => {
  ...
  return (
    <div className='text-black px-4'>
      ...
      {answerList.map((answer: answerMultiRightType) => (
        <Answer
          key={answer.id}
          answer={answer}
...
interface AnswerProps {
  answerType: number;
  answer: answerMultiRightType;
  handleDeleteAnswer: (id: string) => void;
  handleChangeAnswer: (id: string, data: answerMultiRightType) => void;
}
const Answer = ({ answerType, answer, handleDeleteAnswer, handleChangeAnswer }: AnswerProps) => {
  const [answerState, setAnswerState] = useState<answerMultiRightType>(answer);
      ...
      <Checkbox
        ...
        onChange={(e) =>
          setAnswerState((pre: any) => {
            return { ...pre, is_true: e.target.checked };
          })
        }
      />
      <input
...
export default memo(MultiChoiceMultiRight);
